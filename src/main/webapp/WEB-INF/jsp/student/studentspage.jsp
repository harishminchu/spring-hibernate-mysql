...
<script type="text/javascript">
	angular.module('getUttar', []).controller('uttaraCtrl',
			[ '$scope', '$http', function($scope, $http) {
				$http.get('studentJSON').success(function(uttarasList) {
...
<body ng-app="getUttar">
	<div class="container">
		...
		<div class="widget">
			<div class="widget-head">
				<div class="pull-left">Student</div>
				<div class="clearfix"></div>
			</div>
						...
						<div class="row-fluid">
							<div class="fuelux" style="margin-top: 15px">
								...
								<table
									...
									<thead>
										<tr>
											<th colspan="7"><c:url var="addUrl"
													value="/login/student/students/add" /> <c:url
													var="sendSmsUrl" value="/login/student/students/sms?id=" />
												<c:url var="sendAllUrl"
													value="/login/student/students/smsall" /> <c:url
													var="editUrl" value="/login/student/students/edit?id=" />
												<c:url var="deleteUrl"
													value="/login/student/students/delete?id=" /> <input
												type="text" ng-model="search"
												class="input-xlarge search-query" placeholder="Name">
												<input type="text" ng-model="search1"
												class="input-xlarge search-query" placeholder="Usn Number">
												<a class="btn btn-primary" href="${addUrl}">Add</a> <a
												class="btn btn-primary" href="${sendAllUrl}">Send To all</a></th>
										</tr>
									...
									<tbody>
										<tr
											ng-repeat="u in uttaras | filter:search |filter:search1| orderBy:'-uttaraSamaya'">
											...
											<td><a href="${sendSmsUrl}{{u.id}}">Send SMS</a></td>
